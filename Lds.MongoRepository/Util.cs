...
namespace Lds.MongoRepository {
 ...
 /*   public static string GetDefaultConnectionString() {
    ...
    private static IMongoDatabase GetDatabaseFromUrl(MongoUrl url) {
      ...
      return database; // WriteConcern defaulted to Acknowledged

      /*
        var connectionString = ConfigurationManager.ConnectionStrings["momenti-mongo"].ConnectionString;
      var client = new MongoClient(connectionString);
      var database = client.GetDatabase("Momenti");
      var users = database.GetCollection<ApplicationUser>("SiteUsers");
      var roles = database.GetCollection<IdentityRole>("SiteRoles");
    ...
    /// </summary>
    /// <typeparam name="T">The type to get the collection of.</typeparam>
    /// <param name="database">The database to get from</param>
    /// <param name="collectionName">The name of the collection to use.</param>
    /// <returns>Returns a MongoCollection from the specified type and connectionstring.</returns>
    public static IMongoCollection<T> GetCollectionFromDatabase<T>(IMongoDatabase database, string collectionName)
        where T : IEntity<U> {
    ...
    /// <param name="database">The database to get from</param>
    /// <returns>Returns a MongoCollection from the specified type and connectionstring.</returns>
    public static IMongoCollection<T> GetCollectionFromDatabase<T>(IMongoDatabase database)
        where T : IEntity<U> {
